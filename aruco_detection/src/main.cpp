...
//***************carry out ArUco detection*****************************************************//
		...
		// cv::aruco::detectMarkers(im, dictionary, markerCorners, markerIds, parameters, rejectedCandidates);
		...
		if(markerIds.size() > 0){
			if (markerIds.at(0)>500)
			{
				...
				cv::aruco::detectMarkers(gray_flip, dictionary, markerCorners, markerIds, parameters, rejectedCandidates);
				use_flip = true;
				// cout<<"ArUco"<<markerIds.at(0)<<"\n";
			}
		}
	   ...
	   if (!(markerIds.empty()) && use_flip)
       {
          ...
          for (int l=0;l<markerCorners.size();l++)
          {
            ...
            for (int ll=0;ll<4;ll++)
            {
				...
				for (int mm =0; mm <4; mm++) 
				{
				  if (range_image.isObserved (round(range_image_i.width-markerCorners.at(l)[ll].x),round(markerCorners.at(l)[ll].y+mm)))
				  {
					 if (range_image.isObserved (round(range_image_i.width-markerCorners.at(l)[ll].x),round(markerCorners.at(l)[ll].y-mm)))

					 {

						range_image.calculate3DPoint(round(range_image_i.width-markerCorners.at(l)[ll].x),round(markerCorners.at(l)[ll].y+mm),point_up);
                        range_image.calculate3DPoint(round(range_image_i.width-markerCorners.at(l)[ll].x),round(markerCorners.at(l)[ll].y-mm), point_down);
                        ratio = point_down.range/point_up.range;
                        Eigen::Vector3f temp_vertex(point_up.x*(1/(1+ratio))+point_down.x*(ratio/(1+ratio)), 
                        point_up.y*(1/(1+ratio))+point_down.y*(ratio/(1+ratio)), point_up.z*(1/(1+ratio))+point_down.z*(ratio/(1+ratio)));
				...
				//   if (range_image.isObserved (round(range_image_i.width-markerCorners.at(l)[ll].x),round(markerCorners.at(l)[ll].y)))
					 
				// 	 {//if the vertex is observed

                //     	range_image.calculate3DPoint(round(range_image_i.width-markerCorners.at(l)[ll].x),round(markerCorners.at(l)[ll].y), rangePt_ap);
